...
class ParkingAgent:
        ...
        for episode in range(num_episodes):
            ...
            for t in range(self.max_timesteps):
                ...
                with torch.no_grad():
                    ...
                    action = self.actor(state_tensor).to(self.device).numpy()[0]
    ...
    def load_model(self, path='saved_models'):
        self.actor.load_state_dict(torch.load(f'{path}/actor.pth', map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))
        self.critic.load_state_dict(torch.load(f'{path}/critic.pth', map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu')))
